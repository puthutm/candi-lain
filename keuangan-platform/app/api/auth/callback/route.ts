...
export async function GET(req: Request) {
  try {
    ...
    // 3. Resolve user role for keuangan-platform
    const appList = await db
      .select()
      .from(ssoApplications)
      .where(eq(ssoApplications.clientId, "keuangan-platform"))
      .limit(1);

    let role = "mahasiswa"; // default fallback
    ...
    cookieStore.set("keuangan_user", JSON.stringify({
      userId: user.id,
      name: user.fullName,
    ...
    const dashboardTarget = role === "mahasiswa" ? "/skeum" : "/skeu";
    const redirectUrl = new URL(dashboardTarget, req.url);
