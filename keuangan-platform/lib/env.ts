...
/**
 * Environment variables schema with validation for Keuangan Platform
 */
const envSchema = z.object({
  ...
  NEXT_PUBLIC_APP_URL: z.string().default("http://localhost:3005"),
  NEXT_PUBLIC_API_URL: z.string().default("http://localhost:3005/api"),

  // SSO OAuth2 Configuration
  SSO_OAUTH_CLIENT_ID: z.string().default("keuangan-platform"),
  ...
  SSO_OAUTH_CALLBACK_URL: z.string().default("http://localhost:3005/api/auth/callback"),
  ...
  NEXT_PUBLIC_APP_NAME: z.string().default("SKEU"),
...
function parseEnv() {
  try {
    ...
    console.error("❌ Invalid environment variables for Keuangan Platform:");
