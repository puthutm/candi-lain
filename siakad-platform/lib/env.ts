...
/**
 * Environment variables schema with validation for SIAKAD Platform
 */
const envSchema = z.object({
  ...
  SSO_OAUTH_CLIENT_ID: z.string().default("siakad-platform"),
  ...
  SSO_OAUTH_CALLBACK_URL: z.string().default("http://localhost:3003/api/auth/callback"),
  ...
  NEXT_PUBLIC_APP_NAME: z.string().default("SIAKAD"),
  NEXT_PUBLIC_LOGO_URL: z.string().optional(),

  // Cross-Platform Service URLs
  LMS_BASE_URL: z.string().default("http://localhost:3004"),
  LMS_WEBHOOK_URL: z.string().default("http://localhost:3004/api/webhooks/siakad"),
...
function parseEnv() {
  try {
    ...
    console.error("❌ Invalid environment variables for SIAKAD Platform:");
