...
export type DosenTab = "beranda" | "jadwal" | "nilai" | "krs_perwalian";

interface DosenSidebarProps {
  activeTab: DosenTab;
  setActiveTab: (tab: DosenTab) => void;
...
export default function DosenSidebar({
  ...
  const menuItems = [
    { id: "beranda", label: "Beranda Dosen", icon: "🏠" },
    { id: "jadwal", label: "Jadwal Mengajar", icon: "📅" },
    { id: "nilai", label: "Input Nilai Kuliah", icon: "📋" },
    { id: "krs_perwalian", label: "Persetujuan KRS Perwalian", icon: "🛡️", badge: submissionsCount },
  ];
        ...
        <div className="px-6 py-4 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden flex-1">
              <h3 className="font-bold text-white truncate text-sm">{lecturerName}</h3>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
                setActiveTab(item.id as DosenTab);
