...
export async function GET(req: Request) {
  try {
    ...
    // 3. Resolve user role for hris-platform
    const appList = await db
      .select()
      .from(ssoApplications)
      .where(eq(ssoApplications.clientId, "hris-platform"))
      .limit(1);

    let role = "pegawai"; // default fallback
    ...
    cookieStore.set("hris_user", JSON.stringify({
      userId: user.id,
      name: user.fullName,
    ...
    const dashboardTarget = role === "pegawai" ? "/portal" : "/admin";
