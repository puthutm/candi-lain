...
export type TabType = "dashboard" | "tagihan" | "data" | "ujian" | "pengumuman";
...
export default function ApplicantSidebar({
  ...
  const menuItems = [
    { id: "dashboard", label: "Dashboard Alur PMB", icon: "📊" },
    { id: "tagihan", label: "Tagihan & Pembayaran", icon: "💳" },
    { id: "data", label: "Biodata & Berkas Syarat", icon: "📝" },
    { id: "ujian", label: "Ujian CBT Online", icon: "💻" },
    { id: "pengumuman", label: "Hasil Seleksi & Kelulusan", icon: "📜" },
  ];
        ...
        <div className="px-6 py-4 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#FED524] border-2 border-white/20 shadow-md flex items-center justify-center font-bold text-[#0f487b]">
              {(candidateName || "CM").slice(0, 2).toUpperCase()}
            </div>
            <div className="overflow-hidden flex-1">
              <h3 className="font-bold text-white truncate text-sm">{candidateName || "Calon Mahasiswa"}</h3>
              <p className="text-[10px] text-[#FED524] font-bold tracking-wider uppercase font-mono">
                No. Reg: {candidateRef || "REG-2026-001"}
              </p>
            </div>
          </div>
        </div>
