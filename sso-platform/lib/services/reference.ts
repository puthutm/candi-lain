...
export class ReferenceDataService {
  ...
  static async createItem(params: {
    ...
    // Check size limit on extraValue JSON
    ...
    // Check code uniqueness per category
    const existing = await db
      .select()
      .from(refItems)
      .where(
    ...
    // Check parent matching category
    if (params.parentId) {
  ...
  static async updateItem(
    ...
    // Check parent circular dependency and category matching
  ...
  static async assignUserToOrganization(params: {
    ...
    // Check position item validity (if provided)
    ...
    await db.transaction(async (tx) => {
      // If primary is requested, reset other organizations to primary = false
      if (isPrimary) {
        await tx
