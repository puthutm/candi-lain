...
export const refCategories = pgTable("ref_categories", {
  ...
  code: text("code").unique().notNull(), // Unique category code (e.g., "JABATAN")
...
export const refItems = pgTable("ref_items", {
  ...
  parentId: uuid("parent_id").references((): any => refItems.id, { onDelete: "set null" }), // Self-reference for hierarchy
  code: text("code").notNull(),
  name: text("name").notNull(),
  extraValue: jsonb("extra_value"), // Flexible JSON metadata
...
export const organizations = pgTable("organizations", {
  id: uuid("id").primaryKey().defaultRandom(),
  code: text("code").unique().notNull(), // e.g., "HRD", "FIN"
  name: text("name").notNull(),
  parentId: uuid("parent_id").references((): any => organizations.id, { onDelete: "set null" }), // Self-reference
  type: text("type").notNull(), // "company" | "division" | "department"
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const userOrganizations = pgTable("user_organizations", {
  ...
  positionRefItemId: uuid("position_ref_item_id").references(() => refItems.id, { onDelete: "set null" }), // References ref_items category JABATAN
...
export type NewRefCategory = typeof refCategories.$inferInsert;
export type RefItem = typeof refItems.$inferSelect;
export type NewRefItem = typeof refItems.$inferInsert;
