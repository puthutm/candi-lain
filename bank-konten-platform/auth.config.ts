...
/**
 * Bank Konten Platform Auth.js / NextAuth configuration.
 */
export const authConfig: Parameters<typeof NextAuth>[0] = {
  ...
  providers: [
    {
      ...
      name: "UNSIA SSO",
      type: "oauth",
      clientId: process.env.SSO_OAUTH_CLIENT_ID || "bank-konten-platform",
      clientSecret: process.env.SSO_OAUTH_CLIENT_SECRET || "bank-konten-platform-client-secret-key-2026",
      // NOTE: `issuer` is intentionally omitted to prevent oauth4webapi from
      // attempting OIDC well-known discovery over HTTP, which would trigger the
      // "only requests to HTTPS are allowed" error.
      checks: ["pkce", "state"],
      authorization: {
        url: process.env.SSO_OAUTH_AUTHORIZE_URL || "http://10.10.20.56:3000/oauth/authorize",
        params: { scope: "openid profile email" },
      },
      // Custom token request — bypasses oauth4webapi's HTTPS-only enforcement
      // by using plain fetch() instead of the library's built-in client.
      token: {
      ...
      // Custom userinfo request — same reason: bypass HTTPS-only restriction.
      userinfo: {
  ...
  cookies: {
    sessionToken: { name: "bank-konten.authjs.session-token", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } },
    callbackUrl: { name: "bank-konten.authjs.callback-url", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } },
    csrfToken: { name: "bank-konten.authjs.csrf-token", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } },
    pkceCodeVerifier: { name: "bank-konten.authjs.pkce.code_verifier", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } },
    state: { name: "bank-konten.authjs.state", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } },
    nonce: { name: "bank-konten.authjs.nonce", options: { path: "/", sameSite: "lax", secure: false, httpOnly: true } }
  },
  ...
  callbacks: {
    async redirect({ url, baseUrl }) {
      ...
      // Relative callback URLs (e.g. "/dashboard") must be resolved against baseUrl.
