...
/**
 * Environment variables schema with validation for Bank Konten Platform
 */
const envSchema = z.object({
  ...
  NEXT_PUBLIC_APP_URL: z.string().default("http://localhost:3007"),
  NEXT_PUBLIC_API_URL: z.string().default("http://localhost:3007/api"),

  // SSO OAuth2 Configuration
  SSO_OAUTH_CLIENT_ID: z.string().default("bank-konten-platform"),
  ...
  SSO_OAUTH_CALLBACK_URL: z.string().default("http://localhost:3007/api/auth/callback"),
  ...
  NEXT_PUBLIC_APP_NAME: z.string().default("Bank Konten"),
...
function parseEnv() {
  try {
    ...
    console.error("❌ Invalid environment variables for Bank Konten Platform:");
